...
export default function AdminTeachersScreen() {
  ...
  const maleCount   = teachers.filter((t) => { const g = (t.gender ?? "").toLowerCase(); return g === "male"   || g === "m"; }).length;
  const femaleCount = teachers.filter((t) => { const g = (t.gender ?? "").toLowerCase(); return g === "female" || g === "f"; }).length;

  function openAdd() {
    setEditing(null);
    setFormName(""); setFormEmail(""); setFormPhone(""); setFormGender("Male");
    setModalVisible(true);
  }

  function openEdit(t: Teacher) {
    setEditing(t);
    setFormName(t.name); setFormEmail(t.email); setFormPhone(t.phone ?? "");
    const g = (t.gender ?? "").toLowerCase();
  ...
  async function handleSave() {
    ...
    try {
      ...
      Alert.alert("Error", e?.response?.data?.message ?? "Failed to save teacher.");
    } finally {
      setSaving(false);
    }
  }
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
          {filtered.length === 0 && (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySub}>Try a different search or add a new teacher</Text>
            </View>
          )}
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <View style={styles.cardActions}>
                      <TouchableOpacity
                        ...
                        onPress={(e) => { e.stopPropagation(); openEdit(t); }}
                        activeOpacity={0.75}
                      >
        ...
        <KeyboardAvoidingView style={styles.overlay} behavior={Platform.OS === "ios" ? "padding" : "height"}>
          ...
          <View style={[styles.sheet, { paddingBottom: insets.bottom + 16 }]}>
            ...
            <View style={styles.sheetHeader}>
              <View>
                <Text style={styles.sheetTitle}>{editing ? "Edit teacher" : "Add new teacher"}</Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              >
                ...
                <Text style={styles.saveBtnText}>{editing ? "Save changes" : "Add teacher"}</Text>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  header:    { backgroundColor: "#16A34A", paddingHorizontal: 20, paddingBottom: 20 },
  ...
  genderPill: { borderWidth: 1, borderRadius: 6, paddingHorizontal: 7, paddingVertical: 3 },
  ...
  actionEdit:     { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#DCFCE7", borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6 },
  ...
  saveBtn:     { backgroundColor: "#16A34A", borderRadius: 12, paddingVertical: 14, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 },
