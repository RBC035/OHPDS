...
export default function AdminParentsScreen() {
  ...
  const loadParents = useCallback(async () => {
    try {
      ...
      Alert.alert("Error", e?.response?.data?.message ?? "Failed to load parents.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function handleSave() {
    ...
    try {
      ...
      Alert.alert("Error", e?.response?.data?.message ?? "Failed to save parent.");
    } finally {
      setSaving(false);
    }
  }
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
          {filtered.length === 0 && (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySub}>Try a different search or register a new parent</Text>
            </View>
          )}
        ...
        <KeyboardAvoidingView style={styles.overlay} behavior={Platform.OS === "ios" ? "padding" : "height"}>
          ...
          <View style={[styles.sheet, { paddingBottom: insets.bottom + 16 }]}>
            ...
            <View style={styles.sheetHeader}>
              <View>
                <Text style={styles.sheetTitle}>{editing ? "Edit parent" : "Register new parent"}</Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              >
                ...
                <Text style={styles.saveBtnText}>{editing ? "Save changes" : "Register parent"}</Text>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  cardName: { fontSize: 14, fontWeight: "700", color: "#111827", marginBottom: 5 },
  ...
  actionEdit:     { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#EDE9FE", borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6 },
