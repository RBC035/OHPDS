...
// Parse a stored YYYY-MM-DD into a Date for the picker.
...
export default function AdminStudentsScreen() {
  ...
  const [formDob, setFormDob] = useState(""); // YYYY-MM-DD string
  ...
  // The Date currently selected (or a sensible default for first open).
  ...
  async function handleSave() {
    ...
    if (!formDob.trim()) {
      Alert.alert("Required", "Please select the date of birth.");
      return;
    }
    const payload: StudentPayload = {
      name: formName.trim(),
      gender: formGender,
      dob: formDob.trim(), // already YYYY-MM-DD
    };
    try {
      setSaving(true);
      if (editing) {
      ...
      Alert.alert(
        "Error",
        e?.response?.data?.message ?? "Failed to save student.",
      );
    } finally {
      setSaving(false);
    }
        ...
        >
          ...
          <View style={[styles.sheet, { paddingBottom: insets.bottom + 16 }]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              {/* iOS: render inline spinner when open. Android: dialog. */}
              {showDatePicker && (
                <DateTimePicker
                  value={pickerDate}
                  mode="date"
